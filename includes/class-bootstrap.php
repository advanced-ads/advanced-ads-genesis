...
class Bootstrap {
	...
	public function halt_code(): void {
		...
		if ( ! defined( 'ADVADS_VERSION' ) ) {
			...
			add_action( 'after_plugin_row_' . plugin_basename( AAG_FILE ), [ $this, 'missing_row' ] );
			return;
		}
		...
		if ( version_compare( ADVADS_VERSION, '2.0.0', '<' ) ) {
			...
			add_action( 'after_plugin_row_' . plugin_basename( AAG_FILE ), [ $this, 'compatible_row' ] );
			return;
		}
	...
	public function missing_row(): void {
		$this->print_row(
			__( '<strong>Advanced Ads – Genesis</strong> requires the <strong>Advanced Ads free</strong> plugin to be installed and activated on your site.', 'advanced-ads-genesis' )
	...
	public function compatible_row(): void {
		$this->print_row(
			sprintf(
				...
				__( 'Your version of <strong>Advanced Ads – Genesis</strong> is incompatible with <strong>Advanced Ads %s</strong> and has been deactivated. Please update the plugin to the latest version.', 'advanced-ads-genesis' ),
				ADVADS_VERSION
			)
		);
	}
	...
	public function print_missing_notices(): void {
		...
		$this->print_notices(
			__( 'Important Notice', 'advanced-ads-genesis' ),
			__( 'Addons listed below requires the <strong><a href="https://wpadvancedads.com/?utm_source=advanced-ads&utm_medium=link&utm_campaign=activate-advanced-ads-genesis" target="_blank">Advanced Ads</a></strong> plugin to be installed and activated on your site.', 'advanced-ads-genesis' )
			. '&nbsp;' . $this->get_button(),
			$advads_halt_notices
		);
	...
	public function print_halt_notices(): void {
		...
		$this->print_notices(
			...
			sprintf(
				/* translators: %s: Plugin name */
				__( 'Your versions of the Advanced Ads addons listed below are incompatible with <strong>Advanced Ads %s</strong> and have been deactivated. Please update the plugin to the latest version.', 'advanced-ads-genesis' ),
				ADVADS_VERSION
			),
			$advads_halt_notices
		);
	...
	private function get_button( $type = 'button-primary' ): string {
		...
		$text = __( 'Install Now', 'advanced-ads-genesis' );
		...
		if ( isset( $plugins['advanced-ads/advanced-ads.php'] ) ) {
			...
			$text = __( 'Activate Now', 'advanced-ads-genesis' );
		}
